...
REDIPS.drag = (function () {
	...
	var	init,						// initialization
		...
		hover = {colorTd: 'rgba(133, 20, 75, 0.5)',
				colorTr: 'rgba(133, 20, 75, 0.5)'},
		...
		only = {div: [],			// (array) DIVid -> classname, defined DIV elements can be placed only to the marked table cell with class name 'redips-only'
				cname: 'drag-block-cell',		// (string) class name for marked cells (default is 'redips-only') - only defined objects can be placed there
...
redipsInit = function () {
	var rd = REDIPS.drag,
		divDrag = document.getElementById('redips-drag');
	rd.init();
	rd.event.dropped = function () {
		var div1,
			div2 = [],
